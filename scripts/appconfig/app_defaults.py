...
def main():
    ...
    # 3) Collect existing orchestrator keys
    existing_keys = {
        s.key
        for s in client.list_configuration_settings(label_filter="orchestrator")
    }
    ...
    for key, raw_val in defaults.items():
        ...
        logging.info(f"✔ Created key: {key} (label=orchestrator)")
